...
## 15. Scheduler의 실행 한계 명시 (보강)

본 항목은 Scheduler의 역할 오해를 방지하기 위한 보강 규칙이다.

### 15.1 Scheduler는 “결과 요약”을 만들지 않는다

- Scheduler는 다음 정보를 생성하지 않는다.
    - 장비 운전 요약 문구
    - 혼합 운전 여부 표현
    - 냉방/난방 대수 집계
- 이러한 정보는 **UI 또는 Service 표현 책임**이다.

Scheduler는
- 판단 결과를 “해석”하지 않으며
- 결과를 “요약”하거나 “가공”하지 않는다.

---

### 15.2 에어컨 혼합 운전과 Scheduler의 관계

- 동일 발전소 내 에어컨은
  - 냉방 / 난방 혼합 운전이 발생할 수 있다
- Scheduler는:
  - 혼합 운전 여부를 판단하지 않는다
  - 혼합 운전을 차단하지 않는다
  - 혼합 운전을 정상/비정상으로 구분하지 않는다

에어컨 혼합 운전은
- 정책 위반이 아니며
- Scheduler 개입 대상이 아니다

---

### 15.3 MANUAL 상태와 Scheduler의 관계

- MANUAL 상태 판단은 Scheduler 책임이 아니다
- Scheduler는 항상 동일한 Service 메서드를 호출한다
- MANUAL 상태 차단은 전적으로 Service 책임이다

Scheduler는
- MANUAL 상태를 확인하지 않으며
- MANUAL 여부에 따라 분기하지 않는다

---

### 15.4 요약

- Scheduler는 “언제 실행할지”만 책임진다
- “무엇을 할지 / 하지 않을지”는 Service 책임이다
- Scheduler는 **항상 동일한 호출을 반복 수행하는 실행기**이다
