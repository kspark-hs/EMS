...
#### 금지 사항
- ❌ 셀 내부에 “내부 통신 / 외부 통신” 텍스트 반복
- ❌ 한 셀에 여러 텍스트/배지/아이콘 혼합
- ❌ 좌측·우측 정렬 혼용
- ❌ 상태 컬럼과 다른 정렬 방식 사용
- ❌ 줄바꿈을 이용한 다중 라벨 표현
...
> 통신 두절은 경고가 아닌 **비정상 상태**로 간주한다.

---

### 모바일(Card) UI 기준

모바일 UI에서는 테이블 구조를 사용하지 않는다.

- 내부 통신 / 외부 통신 상태를 **요약 배지 형태**로 표시한다.
- 내부 / 외부 통신은 반드시 구분하여 표시한다.
- 색상 규칙은 PC / Tablet 기준과 동일하게 적용한다.
...
- 통신 상태 판단 책임:
  - `PcsAggregateStatusService`
  - `PcsAggregateStatusSnapshot`
- UI 책임:
  - 전달된 문자열(label) **그대로 표현만 수행**
  - 상태 판단 / OR 조합 / 조건 해석 로직 **금지**

---

### 🔒 본 규칙은 고정이다

- 본 섹션은 **PCS StatusTableCard의 최종 UI 기준**이다.
- UI_CONTRACT 수정 없이 아래 변경을 금지한다.
  - 컬럼 구조 변경 ❌
  - 정렬 방식 변경 ❌
  - 통신 컬럼 통합/분리 ❌
...
## 8. PCS 화면 최종 확정 메모 (2026-01-09)

본 섹션은 PCS 개별 화면 구현 과정에서 **최종 합의된 설계 의도와 고정 규칙**을 기록한다.  
본 섹션의 내용은 **UI_CONTRACT와 동일한 강제력을 가진다.**

---

### 8-1. PCS 화면 봉인 선언 (LOCK)

- PCS 개별 화면(UI / 카드 구성 / UX 흐름)은 **최종 확정**되었다.
- UI_CONTRACT 수정 없이 다음 변경을 **금지**한다.
  - 카드 추가 / 삭제 ❌
  - 카드 순서 변경 ❌
  - 카드 의미 재해석 ❌
  - Aggregate 개념의 임의 재도입 ❌

---

### 8-2. PCS 요약 카드(SummaryCard) 수치 항목 의미 확정

PCS 요약 카드에 포함된 아래 3개 수치는 **의도적으로 함께 표시**된다.
...
#### 설계 의도
- **원인(PV) → 목표(Reference) → 결과(PCS)** 흐름을 한눈에 파악하기 위함
- 단순 수치 나열이 아닌 **운전 판단 보조 정보**

---

### 8-3. PV 유효전력 용어 정의 (재확정)

- 메모리맵 기준
  - PV 발전량
  - PV 출력
  - PV 전력  
  → **모두 “PV 유효전력”으로 단일 통일한다.**

- “PV 유효전력은 Trend 전용이다”라는 해석은 **폐기**한다.
- Summary / Status / Trend에서 **동일한 개념**으로 사용한다.

---

### 8-4. PCS StatusTable 동작 규칙 확정

- PCS 대수는 **관리자 설정값** 기준이다.
- PCS StatusTableCard는
  - `PCS #1 ~ PCS #n`까지 **자동 Row 생성**한다.
- Row 클릭 시:
  - 해당 PCS 번호 기준으로
...
### 8-5. PCS 고장정보 카드 Select 동작 확정

- PCS 고장정보 카드 상단 Select는
  - 현재 PCS 대수 기준으로
  - `PCS #1 ~ PCS #n`까지 **자동 생성**된다.
- Select 변경 시:
  - GET 방식 페이지 이동
  - 선택된 PCS 기준으로 고장 정보 갱신
- StatusTable Row 클릭과 **완전히 동일한 이동 규칙**을 사용한다.

---

### 8-6. Aggregate 개념 사용 제한 (중요)

- PCS 개별 화면에서는
  - **Aggregate 전용 요약 카드 사용을 허용하지 않는다.**
- Aggregate 개념은:
  - 대시보드 설계 시점에
  - 필요 시 **재논의**한다.
- 단, 최종 결과는
  - “사용하지 않을 수도 있음”을 전제로 한다.

---

### 8-7. OperationMode / ControlMode 고정값 사용 이유 (메모)
...
- 향후:
  - 메모리맵 / 설정 연동 시
  - **해당 라인만 교체**한다.
- 상태에 따라 동적으로 변경하는 로직은
  - 현 단계에서 **의도적으로 구현하지 않는다.**
